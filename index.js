...
function startAdvOne(numplay, player1name) {
    if (numplay === "1"){
        if(player1name === ""){
            document.getElementById("playerprompt").innerHTML = "Welcome Adventurer! Let's begin.<br><button id='startgame' onclick='prompt1()'>Start Game</button>";
        } else if(player1name !== ""){
            document.getElementById("playerprompt").innerHTML = "Welcome " + player1name + "! Let's begin.<br><button id='startgame' onclick='prompt1()'>Start Game</button>";
        } else {
...
function prompt1() {
    ...
    firstquest.innerHTML = "You're packing your stuff at your desk when the idea to get the code just comes to you. But there's not really much you can do. At least right now. The only places you can go are the hallway to management, the hallway outside, and the washroom. Where do you want to go?<br>";
    ...
    firstquest.appendChild(startpromptinp);
